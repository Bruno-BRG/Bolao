...
\vspace{0.8cm}
{\Large Stack: Next.js, Vercel, Supabase/Postgres e APIs de Futebol\\}
\vfill
\begin{tcolorbox}[colback=softblue,colframe=primary,width=0.9\textwidth,arc=2mm]
\textbf{Objetivo do documento.} Definir escopo, requisitos, arquitetura, modelo de dados, fluxos, APIs externas, endpoints internos, estratégia de autenticação simples e plano de desenvolvimento para um site de bolão da Copa do Mundo com usuários por nome/senha e palpites salvos em JSONB no Supabase.
\end{tcolorbox}
\vfill
{\large Versão 1.0\\}
...
Este documento propõe um sistema web de bolão da Copa do Mundo executando em \textbf{Next.js} e hospedado na \textbf{Vercel}, com persistência em \textbf{Supabase/Postgres}. O sistema terá cadastro simples por \textbf{nome de usuário e senha}, sem usar Supabase Auth, e armazenará os palpites dos usuários em uma coluna \textbf{JSONB} associada ao usuário. A aplicação também manterá um cache interno dos dados da Copa, vindo de uma API de futebol, para evitar dependência direta e excessiva de chamadas externas.
...
Banco de dados & Supabase Postgres & Postgres gerenciado, JSONB, índices, SQL, backups e boa integração com aplicações serverless. \\
...
\begin{enumerate}
  \item \textbf{Segredos só no servidor}: tokens de API, chave service role do Supabase e segredo de sessão nunca vão para o browser.
  \item \textbf{Banco como fonte da aplicação}: a UI lê dados normalizados/cacheados no Supabase, não diretamente da API externa.
...
RNF-03 & Segredos protegidos & Tokens de APIs e service role do Supabase somente em variáveis de ambiente no servidor. \\
...
\begin{lstlisting}[language=SQL,caption={DDL inicial para Supabase/Postgres}]
...
\subsection{RLS e acesso ao Supabase}

Como o projeto não usará Supabase Auth, há duas opções:

\begin{enumerate}
  \item \textbf{Opção recomendada para MVP}: nunca consultar Supabase direto do browser. Todas as consultas passam pelo servidor Next.js usando variáveis de ambiente privadas. Nesse caso, o service role fica apenas no servidor.
  \item \textbf{Opção avançada}: criar uma camada de policies/RLS customizada com JWT próprio. É mais seguro para acesso direto do cliente, mas aumenta bastante a complexidade.
\end{enumerate}

\begin{okbox}
Para este projeto, use Supabase apenas no servidor. No frontend, chame Server Actions ou Route Handlers do Next.js. Isso permite autenticação custom sem expor credenciais sensíveis no cliente.
\end{okbox}

\section{Autenticação customizada}

O sistema não utilizará Supabase Auth. Ainda assim, segurança mínima é obrigatória.
...
\begin{itemize}
  ...
  \item Criar projeto Supabase e rodar SQL inicial.
  \item Configurar variáveis de ambiente na Vercel.
  \item Implementar cliente Supabase server-side.
\end{itemize}
...
\begin{itemize}
  ...
  \item Supabase. \textit{Managing JSON and unstructured data}. \url{https://supabase.com/docs/guides/database/json}
...
Supabase configurado com SQL inicial. & \(\square\) \\
